...
{
    if (phase == xplm_CommandBegin) LogbookUI::toggle();
    return 1;
}
...
PLUGIN_API void XPluginStop()
{
    ...
    AutoQNH::stop();
    RainBlocker::stop();
    if (s_cmd_logbook) XPLMUnregisterCommandHandler(s_cmd_logbook, CmdLogbook, 1, nullptr);
    if (s_cmd_rain)    XPLMUnregisterCommandHandler(s_cmd_rain,    CmdRain,    1, nullptr);
