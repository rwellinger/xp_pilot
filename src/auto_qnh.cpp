...
{
    if (!s_sealevel_pas) return FLIGHTLEVEL;
    return XPLMGetDataf(s_sealevel_pas) * INHG_PER_PA;
}

static float pilot_qnh()
{
    if (!s_baro_pilot) return FLIGHTLEVEL;
    return XPLMGetDataf(s_baro_pilot);
}

static void set_both_baros(float inhg)
{
    if (s_baro_pilot)   XPLMSetDataf(s_baro_pilot,   inhg);
    if (s_baro_copilot) XPLMSetDataf(s_baro_copilot, inhg);
}
...
void AutoQNH::draw()
{
    ...
    // Hysteresis
    if (s_warning_active) {
        if (drift < THRESHOLD_OFF) s_warning_active = false;
    } else {
        if (drift > THRESHOLD_ON) s_warning_active = true;
    }

    if (!s_warning_active && !s_baro_copilot) return;
    ...
    if (s_baro_copilot) {
        ...
        if (std::fabs(pqnh - cpqnh) > FL_EPSILON) {
            ...
            XPLMDrawString(c, 40, 40, const_cast<char*>("ALTIMETER DISAGREE - PF/PM mismatch"), nullptr, xplmFont_Proportional);
        }
    }
}
...
void AutoQNH::stop()
{
    ...
    if (s_cmd_qnh) XPLMUnregisterCommandHandler(s_cmd_qnh, CmdSetQNH, 1, nullptr);
    if (s_cmd_fl)  XPLMUnregisterCommandHandler(s_cmd_fl,  CmdSetFL,  1, nullptr);
}
