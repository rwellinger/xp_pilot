...
static double get_xp_time()
{
    ...
    if (!dr) dr = XPLMFindDataRef("sim/time/total_running_time_sec");
...
static ImVec4 rating_color(const std::string& r) {
    if (r == "BUTTER!")        return {1.0f, 1.0f, 0.0f, 1.0f};
    if (r == "GREAT LANDING!") return {0.25f,1.0f, 0.25f,1.0f};
    if (r == "ACCEPTABLE")     return {0.0f, 0.8f, 0.0f, 1.0f};
    if (r == "HARD LANDING!")  return {1.0f, 0.5f, 0.0f, 1.0f};
    if (r == "WASTED!")        return {1.0f, 0.13f,0.13f,1.0f};
...
{
    // Toolbar
    if (ImGui::Button("Refresh")) { load_entries(); }
    ImGui::SameLine();
    if (ImGui::Button("Rebuild All Reports")) { FlightLogger::regen_all_reports(); }
    ImGui::Separator();
    ...
    if (s_entries.empty()) {
        ...
        for (int i = 0; i < (int)s_entries.size(); ++i) {
            auto& e = s_entries[i];
            char line[128];
            snprintf(line, sizeof(line), "%-11s %-9s %-5s %s",
                     e.date.c_str(),
                     (e.departure_icao + "-" + e.arrival_icao).c_str(),
                     e.aircraft_icao.c_str(),
            ...
            if (ImGui::Selectable(line, sel)) {
                if (s_selected != i) {
                    ...
                    s_report_html   = FlightLogger::data_dir() + "reports/" +
    ...
    if (!s_detail_loaded || s_selected < 0) {
        ...
        std::string route =
            (fd.departure_icao.empty() ? "?" : fd.departure_icao) +
            "  ->  " +
            (fd.arrival_icao.empty()   ? "?" : fd.arrival_icao);
        ImGui::TextUnformatted(route.c_str());

        char info[256];
        snprintf(info, sizeof(info), "%s  %s-%s UTC  |  %s  %s",
                 fd.date.c_str(),
                 fd.start_utc.empty() ? "?" : fd.start_utc.c_str(),
                 fd.end_utc.empty()   ? "?" : fd.end_utc.c_str(),
        ...
        if (fd.track.size() >= 2) {
            ...
            for (auto& p : fd.track) {
                lat_min = std::min(lat_min, p.lat); lat_max = std::max(lat_max, p.lat);
                lon_min = std::min(lon_min, p.lon); lon_max = std::max(lon_max, p.lon);
            }
            ...
            for (size_t i = 1; i < fd.track.size(); ++i) {
                ...
                dl->AddLine(ImVec2(org.x+x1,org.y+y1), ImVec2(org.x+x2,org.y+y2),
                            IM_COL32(212,212,0,255), 1.5f);
            }
            float dx,dy,ax,ay;
        ...
        if (fd.landings.empty()) {
            ...
            for (size_t i = 0; i < fd.landings.size(); ++i) {
                ...
                if (fd.landings.size() > 1) {
                    char h[32]; snprintf(h, sizeof(h), "-- Landing %zu --", i+1);
                ...
                if (ld.time > 0) {
                    char ts[64]; struct tm* t = gmtime(&ld.time);
                    strftime(ts, sizeof(ts), "  Touchdown: %H:%M:%S UTC", t);
                    ImGui::TextUnformatted(ts);
                }
                char stats[128];
                snprintf(stats, sizeof(stats), "  %.0f fpm  |  %.2f G  |  Float %.1f s",
                ...
                if (ld.wind_status == "CALM") {
                    ...
                    snprintf(wind, sizeof(wind), "  Wind: LIGHT  XW %d kts %s",
                    ...
                    snprintf(wind, sizeof(wind), "  TAILWIND +%d kts  --  WRONG RWY?",
                             std::abs(ld.headwind_kts));
                    ImGui::TextUnformatted(wind);
                    ImGui::PopStyleColor();
                } else {
                    snprintf(wind, sizeof(wind), "  Wind: %d kts | HW %d kts | XW %d kts %s",
                             ld.wind_speed_kts, ld.headwind_kts, xw, ld.crosswind_side.c_str());
                    ImGui::TextUnformatted(wind);
                }
                if (i + 1 < fd.landings.size()) ImGui::Separator();
            }
        }
        ...
        // Delete
        if (!s_confirm_del) {
            if (ImGui::Button("Delete")) s_confirm_del = true;
        } else {
            ...
            if (ImGui::Button("Yes, delete")) {
                if (s_selected >= 0 && s_selected < (int)s_entries.size()) {
                    ...
                    std::remove((FlightLogger::data_dir()+"reports/"+
                                 fn.substr(0,fn.rfind('.'))+".html").c_str());
                    HtmlReport::generate_index(FlightLogger::data_dir());
                }
                load_entries();
            }
            ImGui::SameLine();
            if (ImGui::Button("Cancel")) s_confirm_del = false;
        }
    }
    ImGui::EndChild();
}
...
static int MouseCallback(XPLMWindowID wnd, int x, int y,
                          XPLMMouseStatus status, void*)
{
    ...
    if (status == xplm_MouseDown)  io.MouseDown[0] = true;
    if (status == xplm_MouseUp)    io.MouseDown[0] = false;
...
static XPLMCursorStatus CursorCallback(XPLMWindowID, int, int, void*)
{
    return xplm_CursorDefault;
}

static void KeyCallback(XPLMWindowID, char key, XPLMKeyFlags flags,
                         char vkey, void*, int losing_focus)
{
    if (losing_focus) return;
    ImGuiIO& io = ImGui::GetIO();
    // Printable ASCII
    if (!(flags & xplm_DownFlag)) return;
    if (key >= 32 && key < 127) io.AddInputCharacter((unsigned)key);
    // Basic special keys
    if (vkey == XPLM_VK_BACK)   io.AddKeyEvent(ImGuiKey_Backspace, true);
    if (vkey == XPLM_VK_DELETE) io.AddKeyEvent(ImGuiKey_Delete,    true);
    if (vkey == XPLM_VK_RETURN) io.AddKeyEvent(ImGuiKey_Enter,     true);
    if (vkey == XPLM_VK_ESCAPE) { s_logbook_open = false;
                                   if (s_wnd) XPLMSetWindowIsVisible(s_wnd, 0); }
}
...
void LogbookUI::draw()
{
    ...
    if (!s_logbook_open && !FlightLogger::lb_needs_refresh()) return;
...
void LogbookUI::stop()
{
    if (s_wnd) { XPLMDestroyWindow(s_wnd); s_wnd = nullptr; }
    ImGui_ImplOpenGL2_Shutdown();
    if (s_imgui_ctx) { ImGui::DestroyContext(s_imgui_ctx); s_imgui_ctx = nullptr; }
}
