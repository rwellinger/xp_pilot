...
static std::string rating_color(const std::string& r) {
    if (r == "BUTTER!")        return "#ffff00";
    if (r == "GREAT LANDING!") return "#40ff40";
    if (r == "ACCEPTABLE")     return "#00cc00";
    if (r == "HARD LANDING!")  return "#ff8000";
    if (r == "WASTED!")        return "#ff2020";
    return "#ffffff";
}
...
static std::string landing_card(const LandingData& ld,
                                ...
                                const std::array<int,4>& p)
{
    ...
    if (qa <= 1.0f) pitch_label = "good timing";
    else if (qa <= 2.0f) pitch_label = (ld.pitch_deg < 0) ? "late" : "early";
    else pitch_label = (ld.pitch_deg < 0) ? "too late" : "too early";
    ...
    if (ld.wind_status == "CALM") {
        wind_src = "calm"; wind_hw = "calm"; wind_xw = "&mdash;";
    } else if (ld.wind_status == "LIGHT") {
        char b[128];
        snprintf(b, sizeof(b), "%d kts (light/variable)", ld.wind_speed_kts);  wind_src = b;
        snprintf(b, sizeof(b), "%d kts (light/variable)", ld.wind_speed_kts);  wind_hw  = b;
        snprintf(b, sizeof(b), "%d kts %s", xw_abs, xw_side.c_str());          wind_xw  = b;
    } else if (hw < -5) {
        char b[256];
        snprintf(b, sizeof(b), "<b style=\"color:#ff8000\">%d kts TAILWIND &mdash; WRONG RWY?</b>", std::abs(hw));
        wind_hw = b;
        snprintf(b, sizeof(b), "%d kts from %d&deg; mag", ld.wind_speed_kts, ld.wind_dir_mag); wind_src = b;
        snprintf(b, sizeof(b), "%d kts %s", xw_abs, xw_side.c_str());  wind_xw = b;
    } else {
        char b[256];
        snprintf(b, sizeof(b), "%d kts headwind", hw);                          wind_hw  = b;
        snprintf(b, sizeof(b), "%d kts from %d&deg; mag", ld.wind_speed_kts, ld.wind_dir_mag); wind_src = b;
        snprintf(b, sizeof(b), "%d kts %s", xw_abs, xw_side.c_str());          wind_xw  = b;
    }

    char thresh[128];
    snprintf(thresh, sizeof(thresh),
        "Butter &gt;%d / Great &gt;%d / Acceptable &gt;%d / Hard &gt;%d",
        p[0], p[1], p[2], p[3]);
    ...
    snprintf(buf, sizeof(buf),
        ...
        "</table></div>\n",
        rc.c_str(), esc(ld.rating).c_str(),
        rc.c_str(), ld.fpm,
        ld.g_force, ld.float_time, ld.agl_ft,
        ld.pitch_deg, pitch_label.c_str(),
        ld.pitch_rate,
        esc(ld.flare).c_str(),
        wind_src.c_str(), wind_hw.c_str(), wind_xw.c_str(),
        thresh);
    return buf;
}
...
std::string HtmlReport::generate(const FlightData& fd,
                                  ...
                                  const std::array<int,4>& thresholds)
{
    ...
    if (!fd.track.empty()) {
        for (size_t i = 0; i < fd.track.size(); ++i) {
            char b[32];
            if (i) { js_lats += ','; js_lons += ','; js_alts += ','; js_spds += ','; }
            snprintf(b, sizeof(b), "%.6f", fd.track[i].lat); js_lats += b;
            snprintf(b, sizeof(b), "%.6f", fd.track[i].lon); js_lons += b;
            snprintf(b, sizeof(b), "%d",   fd.track[i].alt_ft); js_alts += b;
            snprintf(b, sizeof(b), "%d",   fd.track[i].spd_kts); js_spds += b;
        }
        auto mid = fd.track[fd.track.size() / 2];
        clat = mid.lat; clon = mid.lon;
    }
    js_lats = "[" + js_lats + "]";
    js_lons = "[" + js_lons + "]";
    js_alts = "[" + js_alts + "]";
    ...
    if (fd.landings.empty()) {
        ...
        for (size_t i = 0; i < fd.landings.size(); ++i) {
            if (fd.landings.size() > 1) {
                char h[64]; snprintf(h, sizeof(h), "<h3>Landing %zu</h3>\n", i+1);
                lcards += h;
            }
    ...
    html << "<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>"
         << esc(fd.departure_icao) << " to " << esc(fd.arrival_icao) << "</title>"
         << "<link rel=\"stylesheet\" href=\"https://unpkg.com/leaflet@1.9.4/dist/leaflet.css\"/>"
         << "<script src=\"https://unpkg.com/leaflet@1.9.4/dist/leaflet.js\"></script>"
         << "<script src=\"https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js\"></script>"
         << FL_CSS << "</head><body>"
         << "<h1>" << esc(fd.departure_icao) << " &rarr; " << esc(fd.arrival_icao) << "</h1>"
         << "<h2>" << esc(fd.date) << " " << esc(fd.start_utc) << "&ndash;" << esc(fd.end_utc)
         << " UTC &bull; " << esc(fd.aircraft_icao) << " " << esc(fd.aircraft_tail) << "</h2>"
         << "<div class=\"stats\">"
         << "<div class=\"stat\"><div class=\"val\">" << fmt_dur(fd.block_time_min) << "</div><div class=\"lbl\">Block Time</div></div>"
         << "<div class=\"stat\"><div class=\"val\">" << fd.max_altitude_ft << " ft</div><div class=\"lbl\">Max Altitude</div></div>"
         << "<div class=\"stat\"><div class=\"val\">" << fd.max_speed_kts << " kts</div><div class=\"lbl\">Max Speed</div></div>"
         << "<div class=\"stat\"><div class=\"val\">" << fd.track.size() << "</div><div class=\"lbl\">Track Points</div></div>"
         << "</div>"
         << "<div id=\"map\"></div>"
         ...
         << "<script>"
         << "var lats=" << js_lats << ",lons=" << js_lons
         << ",alts=" << js_alts << ",spds=" << js_spds << ";"
         << "var map=L.map('map');"
         ...
         << "L.circleMarker(coords[coords.length-1],{radius:6,color:'#ff4040',fillOpacity:1}).bindTooltip('Arrival').addTo(map);"
         << "map.fitBounds(poly.getBounds(),{padding:[20,20]});"
         << "}else{map.setView([" << clat_s << "," << clon_s << "],8);}"
         << "var lb=alts.map(function(_,i){return i*10+'s';});"
         << "new Chart(document.getElementById('ac'),{type:'line',data:{labels:lb,datasets:[{label:'Altitude (ft)',data:alts,borderColor:'#00d4ff',backgroundColor:'rgba(0,212,255,.08)',tension:.3,pointRadius:0,fill:true}]},options:{plugins:{legend:{labels:{color:'#e0e0e0'}}},scales:{x:{ticks:{color:'#888',maxTicksLimit:12},grid:{color:'#333'}},y:{ticks:{color:'#888'},grid:{color:'#333'}}}}});"
         << "new Chart(document.getElementById('sc'),{type:'line',data:{labels:lb,datasets:[{label:'IAS (kts)',data:spds,borderColor:'#ff9900',backgroundColor:'rgba(255,153,0,.08)',tension:.3,pointRadius:0,fill:true}]},options:{plugins:{legend:{labels:{color:'#e0e0e0'}}},scales:{x:{ticks:{color:'#888',maxTicksLimit:12},grid:{color:'#333'}},y:{ticks:{color:'#888'},grid:{color:'#333'}}}}});"
         << "</script></body></html>";
...
void HtmlReport::generate_index(const std::string& data_dir)
{
    ...
    for (auto& fname : fnames) {
        ...
        for (auto& ld : fd.landings) last_rating = ld.rating;

        rows.push_back({ fd.date, fd.start_utc, fd.departure_icao, fd.arrival_icao,
                         fd.aircraft_icao, fd.aircraft_tail,
                         "reports/" + fname.substr(0, fname.rfind('.')) + ".html",
                         last_rating, fd.block_time_min, fd.max_altitude_ft });
    }
    ...
    for (auto& r : rows) {
        ...
        std::string ts = !r.start_utc.empty()
            ? (r.date + " " + r.start_utc + " UTC") : r.date;
        char buf[1024];
        snprintf(buf, sizeof(buf),
            "<tr><td>%s</td><td><b>%s &rarr; %s</b></td><td>%s %s</td><td>%s</td>"
            "<td>%d ft</td><td style=\"color:%s\">%s</td><td><a href=\"%s\">Report</a></td></tr>\n",
            esc(ts).c_str(), esc(r.dep).c_str(), esc(r.arr).c_str(),
            esc(r.ac).c_str(), esc(r.tail).c_str(), fmt_dur(r.dur).c_str(),
            r.alt, c.c_str(), esc(r.rating).c_str(), esc(r.link).c_str());
        tbody << buf;
    }

    std::string idx =
        "<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>Flight Log</title>"
        + std::string(FL_CSS) + "</head><body>"
        "<h1>Flight Log</h1>"
        "<p style=\"color:#888\">" + std::to_string(rows.size()) + " flights</p>"
        ...
        "<tbody>" + tbody.str() + "</tbody></table></body></html>";

    std::ofstream out(data_dir + "index.html");
    if (out.is_open()) out << idx;
}
